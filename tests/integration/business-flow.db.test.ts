...
describe.skipIf(!isLocalDatabase)(
  "database-backed partner finance business flow",
  () => {
    let db: ReturnType<typeof createTestAdminClient>;
    let ownerId = "";
    let financeId = "";
    let bankSellerId = "";
    let bankOrderId = "";
    ...
    it(
      "runs onboarding through a paid bank payout with balanced ledger entries",
      async () => {
        ...
        const approved = await approveSeller(
          ownerId,
          seller.id,
        ...
        const payment = await createAndConfirmManualBankPayment(
          financeId,
          order.id,
        );
        bankPaymentId = payment.id;
        await confirmOrderDelivered(financeId, order.id, "DELIVERY-BANK-1");
        const [commission] = await createCommissionsForOrder(
          financeId,
          order.id,
        );
        ...
        bankCommissionId =
          released.find((row) => row.id === commission.id)?.id ?? "";
        ...
        const [{ data: persistedCommission }, { data: ledger }] =
          await Promise.all([
            db
              .from("commissions")
            ...
            db
              .from("ledger_entries")
              .select("entry_type, amount_cents, payout_id")
    ...
    it(
      "confirms a cash payout with a schema-backed receipt",
      async () => {
        const sellerUser = await createAuthUser({ role: "seller" });
        const seller = await approveSeller(
          ownerId,
          (
            await onboardSeller(sellerUser.id, {
              name: "Cash Seller",
              payout: "cash",
            })
          ).id,
        ...
        const payoutResult = await createCashPayoutAndConfirm(
          financeId,
          seller.id,
          [commission.id],
          "Cash Seller",
        ...
        expect(receipt?.receipt_reference).toBe(
          payoutResult.receiptReference,
        );
      },
      60_000,
    );
    ...
    it(
      "keeps the paid payout and creates compensating refund records",
      async () => {
        const { data: beforePayout } = await db
          .from("payouts")
          .select("id, status")
          .eq("id", bankPayoutId)
          .single();
        ...
        const [
          ...
          db
            ...
            .select("amount_cents")
            .eq("commission_id", bankCommissionId),
          db
            .from("ledger_entries")
            .select("commission_id, entry_type, amount_cents")
            .eq("commission_id", bankCommissionId),
          db
            .from("commissions")
            .select("status")
            .eq("id", bankCommissionId)
            .single(),
        ]);
        ...
        await expect(
          createBankPayoutForCommissions(financeId, bankSellerId, [
            bankCommissionId,
          ]),
